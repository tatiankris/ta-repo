...
//// Ключевое отличие состоит в том, что итератор использует механизм, основанный на извлечении, в отличие от наблюдаемого,
// основанного на отправке — наблюдаемый отправляет значения в наблюдатель. Для итераторов и генераторов потребляющий код управляет темпом потребления.
...
///В главе 4 мы обсудим операторы буферизации, которые можно использовать для временного хранения данных в течение определённого
// периода времени или до выполнения определённого условия, если это необходимо. Но по умолчанию данные отправляются вниз
// по течению сразу после их получения.

///Операторы бывают двух видов: как методы экземпляра или как статические методы наблюдаемого типа.
///Таким образом, большинство операторов в RxJS 5 могут быть вызваны как статические или как методы экземпляра
// (когда мы говорим « экземпляр» , мы имеем в виду их вызов с использованием точки (.) для наблюдаемого экземпляра).
